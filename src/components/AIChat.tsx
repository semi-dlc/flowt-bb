  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hi! I'm the flowt agent. I can help you create a shipment offer or request. Are you: \n looking to ship cargo?\n offering transport capacity?\n\nYou can also upload shipping documents (invoices, CMR, etc.) and I'll extract the details for you.",
    },
  ]);
      ...
      <Card className="h-[700px] flex flex-col saas-card shadow-lg">
        <CardHeader className="border-b border-border bg-gradient-to-r from-primary/10 to-accent/10">
          <CardTitle className="flex items-center gap-3">
            ...
            <div>
              <span className="text-foreground font-semibold text-xl">FLOWT Agent</span>
              <p className="text-xs text-muted-foreground font-normal mt-0.5">AI-Powered Freight Assistant</p>
            </div>
          </CardTitle>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="bg-muted rounded-lg px-4 py-2.5">
                    <p className="text-sm flex items-center gap-1.5">
                      <span
                        className="inline-block w-1.5 h-1.5 bg-muted-foreground rounded-full animate-bounce"
                  ...
                  <div key={idx} className="relative group">
                    <div className="w-20 h-20 rounded border border-border overflow-hidden bg-muted flex items-center justify-center">
                      {file.file.type.startsWith("image/") ? (
                        ...
                        <div className="text-xs text-center p-1 truncate w-full">{file.file.name}</div>
                      )}
                    </div>
                    <button
